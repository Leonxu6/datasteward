...
def test_connection_closes_even_if_cursor_close_raises(monkeypatch):
    connector = SqlServerConnector(Source(name="u8", source_type="sqlserver")); cursor = _CloseFailingCursor(); fake = _IntrospectionConnection(cursor=cursor)
    monkeypatch.setattr(sqlserver_module, "_load_driver", lambda: (object(), "pymssql")); monkeypatch.setattr(connector, "_connect", lambda: fake)
...
def test_read_table_normalizes_driver_rows_to_tuples(monkeypatch):
    connector = SqlServerConnector(Source(name="u8", source_type="sqlserver")); cursor = _ReadCursor(); fake = _IntrospectionConnection(cursor=cursor)
    monkeypatch.setattr(sqlserver_module, "_load_driver", lambda: (object(), "pyodbc")); monkeypatch.setattr(connector, "_connect", lambda: fake)
