...
def _audit_best_effort(*args, **kwargs) -> bool:
    """Persist audit data without turning a completed read into a false business failure."""
    try:
...
def describe_table(principal: Principal, name: str) -> str:
    ...
    try:
        ...
        _audit_best_effort(principal, "describe_table", {"name": name}, "", [], 0, t0, False, "invalid table name")
        return f"ERROR: {exc}。请用 list_tables 查看可用表。"
    t = table_by_name(name)
    if not t or name not in business_table_names():
        _audit_best_effort(principal, "describe_table", {"name": name}, "", [], 0, t0, False, "unknown table")
...
def run_sql(principal: Principal, sql: str) -> str:
    ...
    if guard_err:
        ...
        _audit_best_effort(principal, "run_sql", {"sql": sql}, clean, [], 0, t0, False, reason)
        return guard_err
    ...
    if not decision["allow"]:
        _audit_best_effort(principal, "run_sql", {"sql": sql}, clean, tables, 0, t0, True,
