"""Detect sqlite3.connect() calls that rely on an implicit lock timeout."""
...
def audit_source(source:str)->list[str]:
    ...
    for n in ast.walk(tree):
        if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="sqlite3" and n.func.attr=="connect":
            if len(n.args)<2 and not any(k.arg=="timeout" for k in n.keywords): out.append(f"sqlite3.connect() without explicit timeout on line {n.lineno}")
    return out
