"""Detect signal.alarm()/setitimer() calls that mutate process-wide timers."""
...
def audit_source(source:str)->list[str]:
    ...
    for n in ast.walk(tree):
        if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="signal" and n.func.attr in {"alarm","setitimer"}:
            out.append(f"signal.{n.func.attr}() mutates process timers on line {n.lineno}")
    return out
