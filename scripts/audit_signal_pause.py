"""Detect signal.pause() calls that can suspend a service indefinitely."""
...
def audit_source(source: str) -> list[str]:
    return [f"signal.pause can block indefinitely on line {c.lineno}" for c in iter_calls(source) if call_name(c) in _NAMES]
