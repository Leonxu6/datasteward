...
def audit_source(source: str) -> list[str]:
    return [f"sys.set_coroutine_origin_tracking_depth() mutates process diagnostics on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "sys.set_coroutine_origin_tracking_depth"]
