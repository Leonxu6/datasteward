...
def impact_path(entity_id: str, target_type: str, max_hops: int = 4, limit: int = 20):
    h = max(1, min(int(max_hops), 6))
    lim = max(1, min(int(limit), 50))
    tt = "".join(ch for ch in (target_type or "") if ch.isalnum())
    if not tt:
        return {"mode": "impact_path", "error": "需要 target_type（如 Customer / SalesOrder / Material）"}
    rows = run_read(
