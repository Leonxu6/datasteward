...
def audit_source(source: str) -> list[str]:
    return [f"sys.set_asyncgen_hooks() mutates process async-generator behavior on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "sys.set_asyncgen_hooks"]
