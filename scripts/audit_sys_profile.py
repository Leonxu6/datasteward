"""Detect sys.setprofile() calls that install runtime profiling hooks."""
...
def audit_source(source: str) -> list[str]:
    return [f"sys.setprofile() installs a runtime profiling hook on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "sys.setprofile"]
