...
def audit_source(source:str)->list[str]:
    ...
    for n in ast.walk(tree):
        if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="resource" and n.func.attr in {"setrlimit","prlimit"}:
            out.append(f"resource.{n.func.attr}() mutates process limits on line {n.lineno}")
    return out
