...
def audit_source(source:str)->list[str]:
    ...
    for n in ast.walk(tree):
        ...
        if not (isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=='os' and n.func.attr=='chmod'):continue
        mode=n.args[1]
        if isinstance(mode,ast.Constant) and isinstance(mode.value,int) and mode.value&0o002:out.append(f"world-writable chmod mode {oct(mode.value)} on line {n.lineno}")
    return out
