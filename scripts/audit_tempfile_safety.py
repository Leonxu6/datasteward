"""Detect tempfile.mktemp(), which is vulnerable to races."""
...
def audit_source(s:str)->list[str]:
    ...
    return [f"insecure tempfile.mktemp() on line {n.lineno}" for n in ast.walk(t) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=='tempfile' and n.func.attr=='mktemp']
