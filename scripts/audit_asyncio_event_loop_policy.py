...
def audit_source(source: str) -> list[str]:
    return [f"asyncio.set_event_loop_policy() mutates process async policy on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "asyncio.set_event_loop_policy"]
