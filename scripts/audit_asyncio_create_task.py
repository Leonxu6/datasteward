...
def audit_source(source:str)->list[str]:
    ...
    for n in ast.walk(tree):
        if not isinstance(n, ast.Call):
            continue
        f=n.func
        if isinstance(f,ast.Attribute) and isinstance(f.value,ast.Name) and f.value.id=="asyncio" and f.attr=="create_task" and not any(k.arg=="name" for k in n.keywords): out.append(f"asyncio.create_task() without name on line {n.lineno}")
    return out
