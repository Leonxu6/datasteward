"""Detect process-wide tempfile.tempdir replacement in production modules."""
...
def audit_source(source: str) -> list[str]:
    ...
    for node in iter_assignments(source):
        if "tempfile.tempdir" in assignment_targets(node):
            out.append(f"tempfile.tempdir replacement mutates process temporary-file policy on line {node.lineno}")
    return out
