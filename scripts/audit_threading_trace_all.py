...
def audit_source(source: str) -> list[str]:
    return [f"threading.settrace_all_threads() mutates global tracing behavior on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "threading.settrace_all_threads"]
