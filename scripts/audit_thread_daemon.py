...
def audit_source(source: str)->list[str]:
    ...
    for n in ast.walk(tree):
        if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="threading" and n.func.attr=="Thread" and not any(k.arg=="daemon" for k in n.keywords):
            out.append(f"threading.Thread() without explicit daemon policy on line {n.lineno}")
    return out
