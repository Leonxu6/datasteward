"""Detect gc.set_debug() calls that mutate process-wide GC diagnostics."""
...
def audit_source(source: str) -> list[str]:
    return [f"gc.set_debug() mutates process-wide GC diagnostics on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "gc.set_debug"]
