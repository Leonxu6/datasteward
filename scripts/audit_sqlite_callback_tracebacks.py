...
def audit_source(source: str) -> list[str]:
    return [f"sqlite3.enable_callback_tracebacks() mutates process diagnostics on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "sqlite3.enable_callback_tracebacks"]
