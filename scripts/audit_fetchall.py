"""Detect cursor.fetchall() calls that can materialize unbounded result sets."""
...
def audit_source(source:str)->list[str]:
    ...
    return [f"fetchall() can materialize an unbounded result on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and n.func.attr=="fetchall"]
