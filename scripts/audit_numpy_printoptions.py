"""Detect numpy.set_printoptions() calls that mutate process-wide array rendering."""
...
def audit_source(source: str) -> list[str]:
    return [f"numpy.set_printoptions() mutates global array rendering on line {c.lineno}" for c in iter_calls(source) if call_name(c) in {"numpy.set_printoptions", "np.set_printoptions"}]
