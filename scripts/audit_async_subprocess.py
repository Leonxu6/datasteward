...
def audit_source(source:str)->list[str]:
    ...
    for fn in ast.walk(tree):
        ...
        for n in ast.walk(fn):
            if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=='subprocess' and n.func.attr in _HELPERS:out.append(f"subprocess.{n.func.attr}() inside async function {fn.name} on line {n.lineno}")
    return out
