...
def audit_source(source: str) -> list[str]:
    return [f"multiprocessing.set_start_method() mutates process policy on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "multiprocessing.set_start_method"]
