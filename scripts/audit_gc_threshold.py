"""Detect gc.set_threshold() calls that alter process-wide collection policy."""
...
def audit_source(source: str) -> list[str]:
    return [f"gc.set_threshold() mutates process-wide collection policy on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "gc.set_threshold"]
