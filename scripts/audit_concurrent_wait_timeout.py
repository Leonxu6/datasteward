"""Detect concurrent.futures.wait() calls that can block without a timeout."""
...
def audit_source(source: str) -> list[str]:
    ...
    for call in iter_calls(source):
        if call_name(call) in {"concurrent.futures.wait", "futures.wait"} and len(call.args) < 2 and not has_keyword(call, "timeout"):
            out.append(f"concurrent futures wait without timeout on line {call.lineno}")
    return out
