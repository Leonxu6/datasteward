"""Detect queue constructors that omit a capacity and can grow without bound."""
...
def audit_source(source: str) -> list[str]:
    ...
    for n in ast.walk(tree):
        ...
        f=n.func
        if not (isinstance(f,ast.Attribute) and isinstance(f.value,ast.Name) and f.value.id in {"queue","asyncio"} and f.attr in {"Queue","LifoQueue","PriorityQueue"}): continue
        if not n.args and not any(k.arg in {"maxsize"} for k in n.keywords): out.append(f"unbounded {f.value.id}.{f.attr}() on line {n.lineno}")
    return out
