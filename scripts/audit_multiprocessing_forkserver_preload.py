...
def audit_source(source: str) -> list[str]:
    return [f"multiprocessing.set_forkserver_preload() mutates child import policy on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "multiprocessing.set_forkserver_preload"]
