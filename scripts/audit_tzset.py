"""Detect time.tzset(), which mutates process-wide local timezone behavior."""
...
def audit_source(source:str)->list[str]:
    ...
    return [f"time.tzset() mutates process timezone on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="time" and n.func.attr=="tzset"]
