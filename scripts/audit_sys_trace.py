"""Detect sys.settrace() calls that install process/thread tracing hooks."""
...
def audit_source(source: str) -> list[str]:
    return [f"sys.settrace() installs a runtime tracing hook on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "sys.settrace"]
