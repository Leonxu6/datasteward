"""Detect requests.Session() construction that needs an explicit ownership lifecycle."""
...
def audit_source(source:str)->list[str]:
    ...
    return [f"requests.Session() needs explicit lifecycle on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="requests" and n.func.attr=="Session"]
