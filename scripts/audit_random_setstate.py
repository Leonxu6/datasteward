"""Detect random.setstate() calls that replace the module-global RNG state."""
...
def audit_source(source: str) -> list[str]:
    return [f"random.setstate() replaces module-global RNG state on line {c.lineno}" for c in iter_calls(source) if call_name(c) == "random.setstate"]
