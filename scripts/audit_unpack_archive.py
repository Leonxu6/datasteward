...
def audit_source(source: str)->list[str]:
    ...
    for n in ast.walk(tree):
        if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="shutil" and n.func.attr=="unpack_archive":
            out.append(f"shutil.unpack_archive() on line {n.lineno}; verify extraction stays inside the destination")
    return out
